...
def build_schema(ranges: dict[str, list], current_year: int = 2027) -> DataFrameSchema:
    ...
    return DataFrameSchema(
        {
            "price": Column(float, Check.in_range(p_lo, p_hi), nullable=False, coerce=True),
            "brand": Column(str, nullable=False, coerce=True),
            "model": Column(str, nullable=True, coerce=True),
            "year": Column(int, Check.in_range(y_lo, y_hi), nullable=False, coerce=True),
            "mileage": Column(float, Check.in_range(m_lo, m_hi), nullable=True, coerce=True),
        },
...
def validate_schema(df, ranges: dict[str, list], current_year: int = 2027) -> CheckResult:
    ...
    try:
        ...
        n = int(len(fc))
        sample = [{k: str(v) for k, v in row.items()} for row in fc.head(5).to_dict("records")]
        return CheckResult("schema", False, "ERROR", n, len(df), f"{n} schema violation(s)", sample)
